...
"""
Applies notch filter to the whole dataset.
Returns new dataset with transformed data.
"""
function apply_notch_filter(data::RawEEG; fw=50, bw=0.12) 
    fs = data.info["sampRate"][1]
    no_channels = size(data.data)[2]
    no_samples = size(data.data)[1]
    new_signals = Array{Float32}(undef, (no_samples,no_channels))
    for channel in 1:no_channels
        new_signals[:,channel] = notch_filter(data.data[:,channel],fw=fw, fs=fs, bw=bw)
    end
    new_data = RawEEG(data.info, new_signals, data.status, data.lowTrigger, data.highVector, data.chans)
    return new_data
end
...
"""
Applies highpass filter to the whole dataset.
Returns new dataset with transformed data.
"""
function apply_highpass_filter(data::RawEEG, fw; hw=511)
    fs = data.info["sampRate"][1]
    no_channels = size(data.data)[2]
    no_samples = size(data.data)[1]
    new_signals = Array{Float32}(undef, (no_samples,no_channels))
    for channel in 1:no_channels
        new_signals[:,channel] = highpass_filter(data.data[:,channel], fw, fs=fs, hw=hw)
    end
    new_data = RawEEG(data.info, new_signals, data.status, data.lowTrigger, data.highVector, data.chans)
    return new_data
end